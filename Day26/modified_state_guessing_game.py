...
while len(guessed_states) < 50:  # 50 states
    ...
    if answer_state in all_states:
        ...
        t.hideturtle()
        t.penup()
        state_data = data[data.state == answer_state]  # returns the info of the state that we chose as answer_state
        xcor_states = int(state_data.x)
        ycor_states = int(state_data.y)
        t.goto(xcor_states, ycor_states)  # state_data.x ,state_data.y
